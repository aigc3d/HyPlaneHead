...
@persistence.persistent_class
class TriGridGenerator(torch.nn.Module):
    def __init__(self,
        ...
        **synthesis_kwargs,         # Arguments for SynthesisNetwork.
        ...
        self.plane_channel_w_weight = 32
        self.backbone = StyleGAN2Backbone(z_dim, c_dim, w_dim, img_resolution=rendering_kwargs['triplane_size'], 
                                        img_channels=self.plane_channel_w_weight*rendering_kwargs['multiplane_num']*rendering_kwargs['triplane_depth'], mapping_kwargs=mapping_kwargs, **synthesis_kwargs)
        self.superresolution = dnnlib.util.construct_class_by_name(class_name=rendering_kwargs['superresolution_module'], channels=32, img_resolution=img_resolution, 
        ...
        self.t_mapping = MappingNetwork(z_dim=z_dim, c_dim=25, w_dim=3, num_ws=1, last_activation='linear', lr_multiplier=1.0, **mapping_kwargs)
    ...
    def synthesis(self, ws, c, neural_rendering_resolution=None, update_emas=False, 
        ...
        planes = planes.view(len(planes), self.rendering_kwargs['multiplane_num'], self.plane_channel_w_weight * self.rendering_kwargs['triplane_depth'], planes.shape[-2], planes.shape[-1]) 
        ...
        out = {'image': sr_image, 'image_raw': rgb_image, 'image_depth': depth_image, "image_mask": mask_image, "planes": planes}
    ...
    def sample(self, coordinates, directions, z, c, truncation_psi=1, truncation_cutoff=None, update_emas=False, **synthesis_kwargs):
        ...
        planes = self.backbone.synthesis(ws, update_emas=update_emas, **synthesis_kwargs)
        planes = planes.view(len(planes), self.rendering_kwargs['multiplane_num'], 32 * self.rendering_kwargs['triplane_depth'], planes.shape[-2], planes.shape[-1])
        return self.renderer.run_model(planes, self.decoder, coordinates, directions, self.rendering_kwargs)
    def forward(self, z, c_cond, c_render=None, truncation_psi=1, truncation_cutoff=None, neural_rendering_resolution=None, update_emas=False, cache_backbone=False, use_cached_backbone=False, **synthesis_kwargs):
        ws = self.mapping(z, c_cond, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas)
        if c_render is not None:
            c_cam = c_render[:,:25]
        else: 
            c_cam = c_cond[:,:25]
        c_cam, delta_c = self.apply_delta_c(z, c_cam, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas)
        return self.synthesis(ws, c_cam, update_emas=update_emas, neural_rendering_resolution=neural_rendering_resolution, cache_backbone=cache_backbone, use_cached_backbone=use_cached_backbone, **synthesis_kwargs)




##########################################  Triplane+SingSph flatten 2x2 (TriPlaneGenerator_flatten)

@persistence.persistent_class
class TriPlaneSingSphGenerator_flatten(TriGridGenerator):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        **synthesis_kwargs,         # Arguments for SynthesisNetwork.
        ...
        super().__init__(z_dim, c_dim, w_dim, img_resolution, img_channels, sr_num_fp16_res, mapping_kwargs, \
                        rendering_kwargs, sr_kwargs, **synthesis_kwargs)
        self.backbone = StyleGAN2Backbone(z_dim, c_dim, w_dim, img_resolution=rendering_kwargs['triplane_size']*2, 
                                img_channels=self.plane_channel_w_weight*rendering_kwargs['multiplane_num']*rendering_kwargs['triplane_depth'], mapping_kwargs=mapping_kwargs, **synthesis_kwargs)